...
class UtilsController extends Controller
{
	...
	static function formatDateCourte( $date ) {
  	...
  	static function formatDateLongue( $date ) {
    ...
    static function formatDateLongueSepSlash( $date ) {
	...
	static function getDateCourte(){		
		return date("d/m/Y");
	}
	
	static function bissex($an){
	...
	static function getNbJoursParMois($an){
		if (self::bissex($an)) {
	...
	static function joursem($an, $mois, $jour){
	...
	static function premierJourOuvre($mois,$an){		
		/*retourne ????*/
		$numJour = 1 ;
		$index = self::joursem($an,$mois,$numJour) ;
		$nomJour = self::donneJour($index) ;
		if (strcmp($nomJour,"Samedi")==0) return 3 ;
		else if (strcmp($nomJour,"Dimanche")==0) return 2 ;
		else return 1 ;
	}

	static function dernierJourOuvrePremiereSemaine ($mois,$an) {
	...
	static function dernierJourOuvreMois ($mois,$an) {
		$nbJoursParMois = self::getNbJoursParMois($an);
		$nbJoursMois = $nbJoursParMois[$mois-1];
		$index = self::joursem($an,$mois,$nbJoursMois);
		$nomJour = self::donneJour($index);
		if (strcmp($nomJour,"Samedi")==0) return $nbJoursMois-1;
		else if (strcmp($nomJour,"Dimanche")==0) return $nbJoursMois-2;
		else return $nbJoursMois ;
	}

	static function dernierJourMois ($mois,$an) {
    return self::nbJoursMois($mois,$an);
	}
	
	static function estJourWE ($jour,$mois,$an) {	
	...
	static function identifierJourFeries($an){
	...
	static function lundiDePaques($an){
	...
	static function estJourFerie ($jour,$mois,$an) {
		$resultat = false ;
	...
	static function nbJoursOuvresParMois ($mois,$an) {
	...
	static function nbJoursMois ($mois,$an) {
    $nbJours = date("t",mktime(0, 0, 0, $mois, 1, $an));
    return $nbJours;
	}
	
	static function nbSemainesParMois($mois, $an){
		$nbJoursMois = getNbJoursParMois($an);
		$nbSemaine = ceil($nbJoursMois[$mois - 1]/7);
		return $nbSemaine;		
	}
	
	static function isDateValide($aDate){
