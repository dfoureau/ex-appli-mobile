...
class CongesPeriode extends React.Component {
  ...
  setInitialValues() {
    ...
    if (params.idPeriod == null) {
      ...
      let date1 = moment(
          parentState.periods[params.idPeriod].dateDu,
          "YYYY-MM-DD HH:mm:ss"
        ),
        date2 = moment(
          parentState.periods[params.idPeriod].dateAu,
          "YYYY-MM-DD HH:mm:ss"
        );
  ...
  savePeriod(idPeriod) {
    ...
    typeabs =
      arrTypeAbs.find(item => {
        return item.code == this.state.absence;
      }).idTypeAbs || 0;
    ...
    let periodToSave = {
      ...
      dateDu: moment(
        this.state.date1 + " " + hour1,
        "DD-MM-YYYY HH:mm:ss"
      ).format("YYYY-MM-DD HH:mm:ss"),
      dateAu: moment(
        this.state.date2 + " " + hour2,
        "DD-MM-YYYY HH:mm:ss"
      ).format("YYYY-MM-DD HH:mm:ss"),
  ...
  handleValidate() {
    ...
    if (
      this.state.absence != null &&
      this.state.absence != "" &&
      this.state.absence != 0
    ) {
      if (
        this.checkPeriode(
          this.state.date1,
